...
const Carousel = ({ images }) => {
  ...
  return (
    <div className="w-[70vw] relative items-center">
      <button onClick={scrollLeft} className="bg-transparent ml-4 absolute top-1/3">
      ...
      <div
        ...
        {images.map((image, index) => (
          <div key={index} className="flex-shrink-0 h-40 w-1/5 p-2 border border-black" style={{ minWidth: 'calc(100% / 5)' }}>
            {/* Place your content here */}
            <img src={image} alt={`Slide ${index}`} className="w-full h-full object-cover" />
          </div>
        ))}
      </div>
      <button onClick={scrollRight} className="absolute top-1/3 mr-4 right-0 bg-transparent">
